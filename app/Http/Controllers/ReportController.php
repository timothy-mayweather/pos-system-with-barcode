...
class ReportController extends BaseController
{
    ...
    //----------------- Sales Chart js -----------------------\\
    public function choose_date_format(){
        return (config('database.default') === "mysql")?"DATE_FORMAT(date,'%Y-%m-%d')":"STRFTIME('%Y-%m-%d',date)";
    }
    ...
    public function SalesChart()
    {
        ...
        $sales = Sale::where('date', '>=', $date_range)
            ...
            ->groupBy(DB::raw($this->choose_date_format()))
            ->orderBy('date', 'asc')
            ->get([
                DB::raw(DB::raw($this->choose_date_format()." as date")),
    ...
    public function ProfitAndLoss(request $request)
    {
        ...
        foreach($product_sale_data as $key => $product_sale){
            ...
            foreach ($product_purchase_data as $key1 => $product_purchase) {
    ...
    public function get_sales_by_product(request $request)
    {
        ...
        $sale_details_data = SaleDetail::with('product','sale','sale.client','sale.warehouse')
            ->where(function ($query) use ($request, $ShowRecord) {
                if (!$ShowRecord) {
